...
/*   Updated: 2024/06/27 16:29:01 by gromiti          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	*get_cheap(int *a, int *a_n, int *b, int *b_n)
{
	...
	moves[0] = get_r_r(ind(a[0], a, a_n), spot(a[0], b, b_n));
	cheap[0] = moves[0];
	cheap[1] = a[0];
	cheap[2] = 0;
	while (++i < *a_n)
	{
		moves[0] = get_r_r(ind(a[i], a, a_n), spot(a[i], b, b_n));
		moves[1] = get_rr_r(a_n, ind(a[i], a, a_n), spot(a[i], b, b_n));
		moves[2] = get_rr_rr(a_n, b_n, ind(a[i], a, a_n), spot(a[i], b, b_n));
		moves[3] = get_r_rr(b_n, ind(a[i], a, a_n), spot(a[i], b, b_n));
